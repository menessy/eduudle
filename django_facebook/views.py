...
def splitting(x):
    ...
    for i in x.strip().split(','):
        internal = i.split(':')
        try:
            oo =  internal[0].strip().encode().replace("u'",'').replace("'",'')
            ooo = internal[1].strip().encode().replace("u'",'').replace("'",'')
            if ooo == 'https' or ooo == 'http':
                result[oo] = ooo + ':' + internal[2].strip().encode().replace("u'",'').replace("'",'')
            else:
                result[oo] = ooo
        except:
            pass
...
def _connect(request, facebook_login):
    ...
    if facebook_login:
        ...
        #if not xt[0]:
        #    request.META['authenticated2'] = str(user.email)
        #else:
        #    request.META['authenticated2'] = xt[1]
        #return HttpResponseRedirect('/coursewall')
